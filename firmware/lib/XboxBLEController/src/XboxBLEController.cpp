...
bool XboxBLEController::scanAndConnect(uint32_t scanTimeMs)
{
  ...
  // Start scanning
  BLE.scanForUuid(XBOX_SERVICE_UUID, false);

  while (true)
  {
    Serial.println("Searching for Xbox controller...");
    ...
    if (device)
    {
      ...
      if (device.connect())
      {
        ...
        if (peripheral.discoverAttributes())
        {
          ...
          {
            Serial.println("Found attributes");
            // Subscribe to notifications using the handle
            if (reportCharacteristic.canSubscribe())
            {
              const bool sucess = reportCharacteristic.subscribe();
              state.connected = sucess;
...
bool XboxBLEController::findInputReportCharacteristic()
{
  ...
  const int charCount = hidService.characteristicCount();
  ...
  for (int i = 0; i < charCount; i++)
  {
    ...
    Serial.print(", ");
    Serial.println(characteristic.canNotify());

    // Check if this is a HID Report characteristic (UUID 0x2A4D)
    if (strcmp(characteristic.uuid(), XBOX_REPORT_UUID) == 0)
    {
...
bool XboxBLEController::isXboxController(BLEDevice &device)
{
  ...
  if (device.hasLocalName())
  {
    ...
    name.toLowerCase(); // Modified in-place

    const int found1 = name.indexOf("xbox");
    const int found2 = name.indexOf("controller");
